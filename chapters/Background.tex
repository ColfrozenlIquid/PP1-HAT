...
\begin{enumerate}[noitemsep]
    ...
    \item When the owner goes out of scope, the value will be dropped \cite[Section 4.1]{RustBook}
\end{enumerate}

1 \& 2. A value can have only one owner at any given time.
\begin{lstlisting}[language=Rust, frame=single, caption={Single ownership in Rust. Multiple ownership is caught at compile time.}, captionpos=b]
...
{
    let s = String::from("Temporary owner"); // `s` is created inside scope
    println!("{}", s);
...
\begin{lstlisting}[language=Rust, frame=single, caption={Explicit unsafe declaration in Rust}, captionpos=b, numbers=left]
let mut num = 5;
...
\begin{lstlisting}[language=C++, frame=single, caption={Multiple owners in C++. Undefined behaviour is not caught at compile time.}, captionpos=b, label={listing:unique_ptr}]
